...
from modules import om_ext_custom_train_functions
...
from modules.om_ext_lpw_stable_diffusion import StableDiffusionLongPromptWeightingPipeline
import modules.om_ext_model_util as model_util
import modules.om_ext_huggingface_util as huggingface_util
import modules.om_ext_sai_model_spec as sai_model_spec
...
def debug_dataset(train_dataset, show_input_ids=False):
    info(f"Total dataset length (steps) / データセットの長さ（ステップ数）: {len(train_dataset)}")
    info("`S` for next step, `E` for next epoch no. , Escape for exit. / Sキーで次のステップ、Eキーで次のエポック、Escキーで中断、終了します")
    ...
    while True:
        ...
        for i, idx in enumerate(indices):
            ...
            info(f"steps: {steps} ({i + 1}/{len(train_dataset)})")

            example = train_dataset[idx]
            if example["latents"] is not None:
                info(f"sample has latents from npz file: {example['latents'].size()}")
            for j, (ik, cap, lw, iid, orgsz, crptl, trgsz, flpdz) in enumerate(
                ...
                if show_input_ids:
                    info(f"input ids: {iid}")
                    if "input_ids2" in example:
                        info(f"input ids2: {example['input_ids2'][j]}")
                if example["images"] is not None:
                    im = example["images"][j]
                    info(f"image size: {im.size()}")
                    ...
                    if "conditioning_images" in example:
                        ...
                        info(f"conditioning image size: {cond_img.size()}")
...
"""
Module replacement for acceleration.
"""
...
def verify_training_args(args: argparse.Namespace):
    ...
    if args.cache_latents_to_disk and not args.cache_latents:
        ...
        print("cache_latents_to_disk is enabled, so cache_latents is also enabled / cache_latents_to_disk")
    ...
    if args.adaptive_noise_scale is not None and args.noise_offset is None:
        raise ValueError("adaptive_noise_scale requires noise_offset / adaptive_noise_scale noise_offset")

    if args.scale_v_pred_loss_like_noise_pred and not args.v_parameterization:
        raise ValueError("scale_v_pred_loss_like_noise_pred can be enabled only with v_parameterization / scale_v_pred_loss_like_noise_predはv_parameterization")

    if args.v_pred_like_loss and args.v_parameterization:
        raise ValueError("v_pred_like_loss cannot be enabled with v_parameterization / v_pred_like_loss_parameterization")
    ...
    if args.zero_terminal_snr and not args.v_parameterization:
        print(
            ...
            + " / zero_terminal_snr v_parameterization"
        )
