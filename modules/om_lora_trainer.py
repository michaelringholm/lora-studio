...
class OMLoRATrainer():
  ...
  def configure_globals(s):
    ...
    if "model_file" not in globals(): model_file = None
    if "custom_dataset" not in globals(): custom_dataset = None
    if "override_dataset_config_file" not in globals(): override_dataset_config_file = None
    if "override_config_file" not in globals(): override_config_file = None
    if "optimizer" not in globals(): optimizer = "AdamW8bit"
    if "optimizer_args" not in globals(): optimizer_args = None
    if "continue_from_lora" not in globals(): continue_from_lora = ""
    if "weighted_captions" not in globals(): weighted_captions = False
    if "adjust_tags" not in globals(): adjust_tags = False
    if "keep_tokens_weight" not in globals(): keep_tokens_weight = 1.0
    return
  ...
  def raw_setup(s):
    ...
    # cyberrealistic_v40.safetensors | realcartoon3d_v8.safetensors" | "realcartoonRealistic_v11.safetensors" | aZovyaPhotoreal_v2.safetensors"
    s.model_file="v1-5-pruned-emaonly.safetensors"
    s.model_url=f"https://xyz.com/{model_file}"
    ...
    s.images_folder=os.path.join(s.root_dir, "datasets", s.project_name)
    s.output_folder=os.path.join(s.root_dir, "output", s.project_name)
    s.config_folder=os.path.join(s.root_dir, "config", s.project_name)
    s.log_folder=os.path.join(s.root_dir, "log")
    config_file=os.path.join(s.config_folder, "training_config.toml")
    dataset_config_file = os.path.join(s.config_folder, "dataset_config.toml")
    accelerate_config_file = os.path.join(s.repo_dir, "accelerate_config/config.yaml")
  ...
  def validate_dataset(s):
    ...
    steps_per_epoch = pre_steps_per_epoch/s.train_batch_size
    total_steps = s.max_train_steps or int(s.max_train_epochs*steps_per_epoch)
    ...
    print(f"📉 Divide {pre_steps_per_epoch} steps by {s.train_batch_size} batch size to get {steps_per_epoch} steps per epoch.")
    if s.smax_train_epochs:
      print(f"🔮 There will be {s.max_train_epochs} epochs, for around {total_steps} total training steps.")
    else:
    ...
    if adjust_tags:
      ...
      s.adjust_weighted_tags(folders, keep_tokens, keep_tokens_weight, weighted_captions)
  ...
  def download_model(s):
    ...
    model_folder=f"{s.root_dir}/models"
  ...
  def start_training(s,project_name:str,project_dir:str):
    ...
    if old_model_url != model_url or not model_file or not os.path.exists(model_file):
      oml.progress("Downloading model...")
      if not s.download_model():
        err="The model you selected is invalid or corrupted, or couldn't be downloaded. You can use a civitai or huggingface link, or any direct download link."
        oml.error(err)
        raise Exception(err)
    else:
      oml.warn("Model already downloaded.\n")
    s.create_config()
    print("\n⭐ Starting trainer...\n")
    os.chdir(s.repo_dir)
    # https://huggingface.co/docs/accelerate/basic_tutorials/launch
    import train_network
    train_network.train_network_main(s.root_dir,project_name,model_file,model_folder,dataset_config_file,config_file,s.accelerate_config_file,num_cpu_threads_per_process=1)
